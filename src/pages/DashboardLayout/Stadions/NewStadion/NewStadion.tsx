...
const NewStadion: FC = () => {
  const { data: facilities } = useSWR<IFacilitie[]>(
    "/facilitie/getAll",
    fetcher
  );
  ...
  return (
    <div className="h-max">
      <div className="flex c-gap-10 mb-15">
        <Link to={"/admin/dashboard/stadiums"}>
      ...
      <div className="h-max pb-65">
        <form className={classes.form} onSubmit={onSubmit}>
          ...
          <Input
            value={title_ru}
            onChange={setTitle_ru}
            label="Title (Russian)"
          />
          <Input
            value={title_am}
            onChange={setTitle_am}
            label="Title (Armenian)"
          />
          <Input
            value={address_en}
            onChange={setAddress_en}
            label="Address (English)"
          />
          <Input
            value={address_ru}
            onChange={setAddress_ru}
          ...
          <div className={classes.facilities}>
            {facilities?.map((facilitie) => {
              if (selectedFacilities.find((f) => f.id === facilitie.id))
                return undefined;
              return (
                <div
                  key={facilitie.id}
                  ...
                  onClick={() =>
                    setSelectedFacilities((prev) => [...prev, facilitie])
                  }
                >
                  <img
                    src={BASE_URL + facilitie.img}
                    alt="Facilitie"
                    width={30}
                    height={30}
                  />
          ...
          {imgUrl && (
            <img
              width={200}
              src={
                (imgUrl as string).startsWith("data")
                  ? (imgUrl as string)
                  : BASE_URL + imgUrl
              }
              alt="Stadion"
